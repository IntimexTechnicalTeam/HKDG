namespace HKDG.BLL
{
    public class OrderBLL : BaseBLL, IOrderBLL
    {
        ...
        public async Task<SystemResult> BuildOrder(NewOrder checkout)
        {
            if (checkout != null)
            {
                ...
                foreach (var item in checkout.Items)
                {
                    ...
                    //if (vcode != item.ChargeInfo.Vcode) throw new BLException(HKDG.Resources.Message.SelectAddressAgain);
        ...
        {
            var orderDetail = baseRepository.GetList<OrderDetail>(x => x.OrderId == order.Id);
            foreach (var item in orderDetail)
            {
                var result = inventoryBLL.CheckSalesQty(item.SkuId, item.Qty, item.Qty);
                if (!result.Succeeded)
                {
                    string msg = $"發貨失敗，檢查到該訂單中的商品Sku[{item.SkuId}]的庫存異常，詳細內容：{result.Message}";
        ...
        {
            bool result = true;
            if (cond.DeliveryTrackingInfo != null)
            {
        ...
        private async Task<SystemResult> CreateOrder(NewOrder orderView)
        {
            ...
            //发送MQ计算qty
            await SendOrderCreatedMsg(dbOrder.Id);
        ...
        /// <param name="OrderId"></param>
        /// <returns></returns>
        public async Task<SystemResult> SendOrderCreatedMsg(Guid OrderId)
        {
            var result = new SystemResult();
            var details = (await baseRepository.GetListAsync<OrderDetail>(x => x.OrderId == OrderId)).ToList();
            //订单库存计算队列消息数据
            var calcuateList = await GenCalculateQueueList(details);

            //kol佣金队列消息数据
            //var kolDetails = details.Where(x => x.isKol).ToList();
            //var msgList = KolFlowBLL.CalculateCommissionWhenOrderCreated(kolDetails);

            using (var tran = baseRepository.CreateTransation())
            {
                //await baseRepository.InsertAsync(msgList);
                await baseRepository.InsertAsync(calcuateList);
                tran.Commit();
                result.Succeeded = true;
            }
            ...
            //{
            //    rabbitMQService.PublishMsg(MQSetting.ProxyCommissionQueue, MQSetting.ProxyCommissionExchange, item.Id.ToString());
            //}

            return result;
        }

        async Task<List<CalculateQtyQueue>> GenCalculateQueueList(List<OrderDetail> details)
        {
            var queueList = new List<CalculateQtyQueue>();
            foreach (var item in details)
            {
                ...
                queueList.Add(new CalculateQtyQueue { Id = Guid.NewGuid(), SkuId = item.SkuId, Qty = holdQty, RelatedId = item.Id, QtyType = QtyType.WhenCreateOrder });
            }
            return queueList;
        }
