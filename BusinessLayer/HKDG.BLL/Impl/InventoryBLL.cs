    ...
    public class InventoryBLL : BaseBLL, IInventoryBLL
    {
                ...
                if (recIdList.Any())
                {
                    ...
                    foreach (var recId in recIdList)
                    {
                        ...
                        if (!checkRes.Succeeded)
                        {
                            ...
                            checkRes.Message = HKDG.Resources.Message.InventoryQtyNotNull + "(" + whShow.Name + ")";
                            return checkRes;
                        }
        ...
        public SystemResult IsExsitBathNum(InvTransactionDtlDto cond)
        {
            ...
            if (query != null && query.Any())
            {
                ...
                sysRslt.Message = HKDG.Resources.Message.BatchNumExsit;
            }
            else
            {
        ...
        private SystemResult DeductInvQtyWithReservePreprocess(InventoryReservedDto reserve)
        {
            ...
            var currentInvt = currentInvtLst?.FirstOrDefault() ?? null;
        ...
        private SystemResult AddInvReservedWithSign(InventoryReservedDto reserve, bool isBatch, Guid memberId)
        {
            ...
            InvReservedRepository.DelInvReserved(new InventoryReserved()
            {
                OrderId = reserve.OrderId,
                SubOrderId = reserve.SubOrderId,
                Sku = reserve.Sku,
            ...
            InventoryReserved reservedDetail = new InventoryReserved()
            {
                Id = Guid.NewGuid(),
                OrderId = reserve.OrderId,
                SubOrderId = reserve.SubOrderId,
        ...
        public async Task<SystemResult> CheckSalesQtyAsync(Guid SkuId, int AddQty, int HoldQty)
        {
            ...
            {
                //bool isInventoryEnable = SettingBLL.IsInventoryEnable();
                //if (isInventoryEnable)
                //{
                var invSku = new InventoryReservedDto();
                invSku.Sku = SkuId;

                var totalAvailable = GetTotAvailableInvQty(invSku);

                if (AddQty > 0)//增加產品數量時才判斷庫存，如果小於0表示系釋放產品不需要判斷是否有庫存
                {
                    string key = $"{CacheKey.SalesQty}";
                    if (totalAvailable <= 0)//可銷售數量為0 
                    {
                        var errEnum = OrderErrorEnum.Sellout;
                        //更新Redis缓存中的saleqty    
                        await RedisHelper.HSetAsync(key, SkuId.ToString(), 0);
                        result.Succeeded = false;
                        var error = new SystemError()
                        {
                            Code = (int)OrderErrorEnum.Sellout,
                    ...
                    else
                    {
                        ...
                        if (ReservedQty >= totalAvailable)//ReservedQty大于可销售数时不能购买
                        {
                            result.Message = "库存数量不足";
                            result.Succeeded = false;

                        }
                        else if (totalAvailable < AddQty)
                        {
                            //少於購買增量，不能購買
                            result.Succeeded = false;
                            var error = new SystemError()
                            {
                                Code = (int)OrderErrorEnum.OutOfStock,
                                Description = GetErrorDesription((int)OrderErrorEnum.OutOfStock, CurrentUser.Lang)
                            };
                            result.Message = error.Message;
                            result.ReturnValue = error;
                        }
                        ...
                        else if (totalAvailable < HoldQty)
                        {
                            result.Succeeded = false;
                            var error = new SystemError()
                            {
        ...
        public SystemResult CheckSalesQty(Guid SkuId, int AddQty, int HoldQty)
        {
            var result = new SystemResult { Succeeded = true };
            var sellOutSkuList = GetSelloutSkus();
            if (sellOutSkuList != null && sellOutSkuList.Any(d => d == SkuId.ToString()))
            ...
            else
            {
                ...
                if (AddQty > 0)//增加產品數量時才判斷庫存，如果小於0表示系釋放產品不需要判斷是否有庫存
                {
                    ...
                    if (totalAvailable <= 0)//可銷售數量為0 
                    {
                        var errEnum = OrderErrorEnum.Sellout;
                        //更新Redis缓存中的saleqty    
                        RedisHelper.HSet(key, SkuId.ToString(), 0);
                        result.Succeeded = false;
                        var error = new SystemError()
                        {
                            Code = (int)OrderErrorEnum.Sellout,
        ...
        public List<string> GetSelloutSkus()
        {
            string SalesQtyKey = $"{CacheKey.SalesQty}";
            //优先读缓存
            var cacheData = RedisHelper.ZRangeByScore(SalesQtyKey, "-inf", "0");   //取小于等于0           
