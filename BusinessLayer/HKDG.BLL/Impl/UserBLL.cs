    ...
    public class UserBLL : BaseBLL, IUserBLL
    {
        ...
        public SystemResult CreateAccountForMerchant(string Ids)
        {
            ...
            foreach (var item in merchantIds)
            {
                ...
                {
                    result.Message += HKDG.Resources.Message.MerchantAccountCreated;
                    break;
                }
        ...
        public SystemResult Remove(Guid Id)
        {
            ...
            if (user == null)
            {
                result.Message = HKDG.Resources.Message.UserNotExist;
                return result;
            }
            user.IsDeleted = true;
        ...
        public SystemResult Update(UserDto model)
        {
            ...
            var entity = baseRepository.GetModelById<User>(model.Id);
            if (entity == null || entity.IsDeleted) throw new BLException(HKDG.Resources.Message.UserNotExist);           
            ...
            if (model.Account != entity.Account)
            {
                ...
                if (existSameName) throw new BLException(HKDG.Resources.Message.AccountExist);
            }

            if (model.Email != entity.Email)
            {
                var existSameEmail = baseRepository.Any<User>(d => d.Id != model.Id && d.Email == model.Email && !d.IsDeleted);
                if (existSameEmail) throw new BLException(HKDG.Resources.Message.EmailExist);
            }
            #endregion
            ...
            {             
                ...
                if (!oldMerchantUser) throw new BLException(HKDG.Resources.Message.OldMerchantMbrEmpty);              
            }
